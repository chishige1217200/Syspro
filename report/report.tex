...
本レポートでは，\ref{sec:task}章に示す5つの課題に取り組み，その解答を報告する．使用した
MIPSアセンブリ言語のソースコードは，\ref{sec:makep}章に示す．実行結果はxspimによる結果である．

%--------------------------------------------------------------------%
\section{課題} \label{sec:task}
システムプログラミング1の課題は次の5つである．解答は\ref{sec:answer}章に示す．
\begin{enumerate}
